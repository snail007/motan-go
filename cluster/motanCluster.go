...
func (l *clusterGroupNodeChangeListener) GetIdentity() string {
	...
	if id == "" {
		...
		//       but now it's ok
		id = fmt.Sprintf("%p-%s", l, l.subscribeURL.GetIdentity())
		l.identity.Store(id)
	}
	return id
}
...
func (m *MotanCluster) parseRegistry() (err error) {
	...
	if !ok {
		err = fmt.Errorf("registry not found for cluster: %+v", m.url)
...
const (
	// only if using backupGroupDelimiter like "," can be use as backup group
	// ${idc} type will using groups randomly
...
var DetermineGroupConfig = func(url *motan.URL, registries []motan.Registry) GroupConfigInfo {
	...
	if strings.Contains(url.Group, backupGroupDelimiter) {
		if groupList := motan.TrimSplit(url.Group, backupGroupDelimiter); len(groupList) > 0 {
